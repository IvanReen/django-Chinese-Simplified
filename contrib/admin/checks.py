...
def check_dependencies(**kwargs):
    ...
    # 如果使用默认身份验证后端，则必须安装auth上下文处理器。
    try:
        default_template_engine = Engine.get_default()
    except Exception:
        '''
        跳过这个非关键检查：
         1.如果用户有一个非平凡的TEMPLATES设置，Django找不到默认的模板引擎
         2.如果在加载模板引擎时出现任何问题，为了避免从令人困惑的位置引发异常，Catching ImproperlyConfigured只能为1.但2.要求捕获所有异常。
        '''
    else:
...
class BaseModelAdminChecks:
    ...
    def _check_autocomplete_fields(self, obj):
        """
        检查`autocomplete_fields`是模型字段的列表或元组。
        """
        return (
            list(
                chain.from_iterable(
    ...
    def _check_autocomplete_fields_item(self, obj, model, field_name, label):
        """
        检查`autocomplete_fields`中的项是ForeignKey还是ManyToManyField，并且该项具有定义了search_fields的相关ModelAdmin。
        """
        try:
            ...
            list(
                chain.from_iterable(
                    ...
                    + self._check_prepopulated_fields_value(
                        ...
                        obj.model,
                        val,
                        'prepopulated_fields["%s"]' % field_name,
                    )
